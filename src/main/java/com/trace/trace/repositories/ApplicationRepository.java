...
public interface ApplicationRepository extends JpaRepository<Application, Long> {
    ...
    @Query(value = "SELECT stage FROM stages s JOIN applications_stages ast ON ast.stage_id = s.id JOIN applications a ON ast.application_id = a.id WHERE a.id = ?1 ORDER BY ast.created_at DESC LIMIT 1", nativeQuery = true)
